  ...
  _createClass(Client, [{
    ...
    value: function listObjects(bucketName, prefix, recursive) {
      ...
      readStream._read = function () {
        ...
        _this7.listObjectsQuery(bucketName, prefix, marker, listQueryOpts).on('error', function (e) {
          return readStream.emit('error', e);
        }).on('data', function (result) {
          if (result.isTruncated) {
    ...
    value: function listObjectsV2(bucketName, prefix, recursive, startAfter) {
      ...
      readStream._read = function () {
        ...
        _this8.listObjectsV2Query(bucketName, prefix, continuationToken, delimiter, 1000, startAfter).on('error', function (e) {
          return readStream.emit('error', e);
        }).on('data', function (result) {
          if (result.isTruncated) {
    ...
    value: function presignedUrl(method, bucketName, objectName, expires, reqParams, requestDate, cb) {
      ...
      this.getBucketRegion(bucketName, function (e, region) {
        ...
        var reqOptions = _this10.getRequestOptions({
          method,
          region,
          bucketName,
          objectName,
        ...
        try {
          url = (0, _signing.presignSignatureV4)(reqOptions, _this10.accessKey, _this10.secretKey, _this10.sessionToken, region, requestDate, expires);
        } catch (pe) {
          return cb(pe);
        }
    ...
    value: function presignedPostPolicy(postPolicy, cb) {
      ...
      this.getBucketRegion(postPolicy.formData.bucket, function (e, region) {
        ...
        postPolicy.policy.conditions.push(["eq", "$x-amz-credential", _this11.accessKey + "/" + (0, _helpers.getScope)(region, date)]);
        postPolicy.formData['x-amz-credential'] = _this11.accessKey + "/" + (0, _helpers.getScope)(region, date);

        if (_this11.sessionToken) {
          postPolicy.policy.conditions.push(['eq', '$x-amz-security-token', _this11.sessionToken]);
          postPolicy.formData['x-amz-security-token'] = _this11.sessionToken;
        }

        var policyBase64 = Buffer.from(JSON.stringify(postPolicy.policy)).toString('base64');
        postPolicy.formData.policy = policyBase64;
        var signature = (0, _signing.postPresignSignatureV4)(region, date, _this11.secretKey, policyBase64);
        postPolicy.formData['x-amz-signature'] = signature;
        var opts = {};
        opts.region = region;
        opts.bucketName = postPolicy.formData.bucket;

        var reqOptions = _this11.getRequestOptions(opts);

        var portStr = _this11.port == 80 || _this11.port === 443 ? '' : `:${_this11.port.toString()}`;
    ...
    value: function listParts(bucketName, objectName, uploadId, cb) {
      ...
      var listNext = function listNext(marker) {
        _this12.listPartsQuery(bucketName, objectName, uploadId, marker, function (e, result) {
          if (e) {
            cb(e);
            return;
          }
    ...
    value: function findUploadId(bucketName, objectName, cb) {
      ...
      var listNext = function listNext(keyMarker, uploadIdMarker) {
        _this13.listIncompleteUploadsQuery(bucketName, objectName, keyMarker, uploadIdMarker, '').on('error', function (e) {
          return cb(e);
        }).on('data', function (result) {
    ...
    value: function getUploader(bucketName, objectName, metaData, multipart) {
      ...
      var upload = function upload(query, stream, length, sha256sum, md5sum, cb) {
        ...
        if (!_this14.enableSHA256) headers['Content-MD5'] = md5sum;

        _this14.makeRequestStream({
          method,
          bucketName,
          objectName,
          query,
    ...
    value: function putObjectRetention(bucketName, objectName) {
      ...
      headers['Content-MD5'] = md5digest.toString('base64'); //FIXME minio Server returns 204 but AWS returns 200. So in aws, though the operation is success, error is thrown.
